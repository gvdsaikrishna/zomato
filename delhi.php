...
<div id="resp-search-container" class="search-box-area"></div>
    ...
    <img src="t.jpg" alt="Best Restaurants in India" />
        ...
        echo "
        <marquee scrollamount='15' width='80%' behavior='slide'><h1 class='h-city-home-title' style='color:White;'>

        
            Welcome to Tomato ..... The online food Store<br>Select your Restaurant from Delhi        </h1></marquee>";?>
            </div>
...
$sql = "select res_name,restaurant_regno from  Restaurant where city='Delhi'";
 ...
 while($row = $result->fetch_assoc()) 
  {
          ...
          array_push($res_nm, $tpad);
          $tpad1 = $row['restaurant_regno'];

          array_push($res_reg, $tpad1);
     }
...
while($ctr>=0)
{
  ...
  echo "<h2>".$res_nm[$ctr]."</h2>";
   echo '<form method="post" action="order_now.php">';
   echo '<h3><button type="submit" value="' . $res_reg[$ctr]. '" name="res_id">Order Now&nbsp&nbsp !!</button>';
    echo '</h3></form>';
...
echo '<div align="right"><img src="del.jpg " width="380" height="240" ></div>';
$n = $_SESSION['login_user'];
$sql = "select item_name from history,Restaurant  where customer_id='$n' and Restaurant.restaurant_regno=history.restaurant_regno and Restaurant.city='Delhi'  order by quantity DESC limit 5";
$result = $conn->query($sql);
if($result->num_rows>0)
    {

        echo"<h1>Your Favourites from Delhi based on previous purchases :<br></h1>";
        while($row = $result->fetch_assoc()) 
        {
            echo "<h2>".$row['item_name']."</h2><hr>";
            //echo '<br>';
        }
    }
    else
    {
        echo "<h2>Seems like you have'nt tasted food from Delhi<br>Try it out</h2>";
    }
...
$sql = "select res_name from  Restaurant where city='Delhi'";
$result = $conn->query($sql);
$res = array();
...
while($ctr>=0)
{
  ...
  echo $res[$ctr];
   echo '<form method="post" action="order_now.php">';
   echo '<button type="submit" value="' . $res[$ctr]. '" name="res_n">Remove</button>';
    echo '</form>';
...
//echo $res[1]."<a href='/order_now.php' >Order!!!!!!!</a>";
